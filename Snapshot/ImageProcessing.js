...
class ImageProcessing {
    ...
    init() {
        ...
        this.filters.addContrast = this.addContrast;
    ...
    grayBalance = function () {
        ...
        let ImgCDF = [];
        let newImgData = [];
        let pixels = this.outputData.data;
        let output = this.outputData.data;
    ...
    addContrast = function (value) {
        let pixels = [...this.imgData.data];
        let output = this.outputData.data;
        let alpha = (value + 255) / 255
        // console.log(output);
        // console.log(pixels);
        ...
        for (let i = 0; i < pixels.length; i += 4) {
            ...
            pixels[i] = this.clamp(alpha * (r - 128) + 128)
            pixels[i + 1] = this.clamp(alpha * (g - 128) + 128)
            pixels[i + 2] = this.clamp(alpha * (b - 128) + 128)
